...
def extrair_divisor_inteligente(apres_cmed, unid_proposta):
    ...
    if any(x in unid_proposta for x in ["CX", "CAIXA", "DOS", "DOSE"]): return 1
    if unid_proposta == "ML":
...
def processar_dados(file_proposta, df_cmed, coluna_icms):
    try:
        df_raw = pd.read_excel(file_proposta, header=None) if 'xls' in file_proposta.name else pd.read_csv(file_proposta, sep=None, engine='python', header=None)
        linha_cab = 0
...
with st.sidebar:
    # Logo atualizada
    st.image("https://drogafonte.com.br/wp-content/themes/drogafonte/assets/img/logo.png", width=180)
    st.divider()
    aliquota = st.selectbox("Selecione o ICMS de Destino:", ["PF 12%", "PF 17%", "PF 17,5%", "PF 18%", "PF 19%", "PF 19,5%", "PF 20%", "PF 20,5%", "PF 21%", "PF 22%"], index=7)
    if df_cmed is not None: st.success("Base CMED Carregada")
...
if not st.session_state.tela_resultado:
    ...
    col1.button("⬅️ Voltar", on_click=resetar_app)
    
    if st.session_state.erro:
        st.error(st.session_state.erro)
    else:
        # --- TABELA 1: PREÇOS ---
        st.subheader("🚨 Itens com Preço Acima do Teto")
        df_p = st.session_state.dados_finais
        if df_p.empty:
            st.success("Tudo certo! Nenhum item acima do teto.")
        else:
            ...
            st.download_button("📥 Baixar Divergências em Excel", converter_para_excel(exib_p), "Divergencias_Preco.xlsx")
        ...
        if df_r.empty:
            st.info("Nenhuma inconsistência de registro.")
        else:
            exib_r = df_r[['Col_Item', 'Col_Desc', 'Col_Reg']].copy()
            exib_r['Diagnóstico'] = df_r.apply(lambda x: "Registro Inválido (≠ 13 dígitos)" if len(str(x['Reg_L'])) != 13 else "Não encontrado na CMED (Pode ser RDC/Notificado)", axis=1)
            exib_r.columns = ['Item', 'Descrição', 'Registro Informado', 'Diagnóstico']
            st.dataframe(exib_r, use_container_width=True)
            st.download_button("📥 Baixar Alertas em Excel", converter_para_excel(exib_r), "Alertas_Registro.xlsx")

        # --- GERADOR DE PDF COMPLETO ---
        if st.button("📄 Gerar Relatório PDF Unificado", type="primary"):
            ...
            pdf.cell(0, 10, f"RELATORIO DE DIVERGENCIAS DE PRECO - {st.session_state.aliquota}", ln=True, align='C')
            ...
            st.download_button("💾 Salvar PDF", pdf.output(dest='S').encode('latin-1'), "Relatorio_Auditoria_Completo.pdf", "application/pdf")
