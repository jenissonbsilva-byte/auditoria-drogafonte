...
# 4. FUNÇÃO DE FRACIONAMENTO RECALIBRADA (v2.4)
def extrair_qtd_cmed(apres):
    apres = str(apres).upper()
    
    # REGRA DE OURO: Se não tiver menção a Caixa, Cartucho ou ML, considera 1 UNIDADE
    termos_coletivos = ['CX', 'CAR', 'CT', 'ML', 'AMP', 'FA', 'FR', 'SER', 'BOLS']
    if not any(termo in apres for termo in termos_coletivos):
        return 1
    ...
    if st.button("🚀 Executar Auditoria Oficial", use_container_width=True):
        with st.spinner('Auditando itens...'):
            try:
                ...
                if not achou:
                    st.error("Cabeçalho não identificado.")
                    st.stop()
                ...
                def find_col(nomes, idx):
                    cols = [c for c in df_prop.columns if any(p.upper() in str(c).upper() for p in nomes)]
                    return cols[0] if cols else df_prop.columns[idx]
                ...
                if df_erros.empty:
                    ...
                    pdf.cell(34, 8, "Proposta", 1, 0, 'C', True)
