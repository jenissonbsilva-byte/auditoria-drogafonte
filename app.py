...
with st.sidebar:
    ...
    try: indice_pe = lista_estados.index("PERNAMBUCO (20,5%)")
    except: indice_pe = 0
    ...
    if st.session_state.erro:
        ...
        with tab1:
            ...
            if df_p.empty: 
                ...
                exib_p = df_p[['Col_Item', 'Col_Desc', 'Col_Marca', 'V_Unit', 'PF_Num', 'Divisor', 'Teto_U', 'Diferenca']].copy()
                exib_p.columns = ['Item', 'Descrição', 'Marca/Fabricante', 'Valor Proposta', 'PF CMED', 'Divisor', 'Teto Unit.', 'Diferença']
                st.dataframe(
        ...
        with tab2:
            ...
            exib_t = df_t[['Col_Item', 'Col_Desc', 'Col_Marca', 'V_Unit', 'PF_Num', 'Divisor', 'Teto_U', 'Diferenca', 'Status']].copy()
            exib_t.columns = ['Item', 'Descrição', 'Marca/Fabricante', 'Valor Proposta', 'PF CMED', 'Divisor', 'Teto Unit.', 'Diferença', 'Status']
            st.dataframe(
        ...
        with tab3:
            ...
            if not df_r.empty:
                ...
                st.dataframe(df_r[['Col_Item', 'Col_Desc', 'Col_Marca', 'Col_Reg']], use_container_width=True)
            else:
        ...
        if st.button("📄 Gerar Relatório PDF Final", type="primary", use_container_width=True):
            ...
            if not df_p.empty:
                ...
                for h in st.session_state.cabecalho_pdf: 
                    if str(h).strip():
                ...
                pdf.cell(45, 8, "Marca/Fabricante", 1, 0, 'C', True)
                pdf.cell(22, 8, "Proposta", 1, 0, 'C', True)
                pdf.cell(22, 8, "PF CMED", 1, 0, 'C', True)
                pdf.cell(12, 8, "Div.", 1, 0, 'C', True)
            ...
            if not df_r.empty:
                ...
                pdf.cell(55, 8, "Marca/Fabricante", 1, 0, 'C', True)
                pdf.cell(45, 8, "Registro Informado", 1, 1, 'C', True)
